...
scenarios=data.frame(np=c(10,10,10,10,20,20,20,20),lamp=c(5,5,12.5,12.5,5,5,12.5,12.5),lami=rep(0.3,8), attr=rep(c(-1, -0.1), 4))
setwd("D:/Workspace/emmetr/newest_october20_awd/Ben Analyses/")
#Scenario 1
scen=1
true=c(5,0.034,4.5,0.3,10/60,10,NA,NA)
modes=covers=groupmodes=groupcovers=groupeffective=groupgelmans=effective=gelmans=matrix(NA,nrow=100,ncol=8)
HPDs=creds=groupcreds=array(NA,dim=c(100,8,2))
storeSV=storenp=storeNi=storeni=ind_recaptures=pack_recaptures=rep(NA,100)
for(i in 1:100){
  load(paste("sims/np", scenarios[scen,1], "/lamp", scenarios[scen,2], "/lami", scenarios[scen,3], "/attr", abs(scenarios[scen, 4]), "/rep", i, "/knownidhmmresults.RData", sep=""))
  if(all(is.na(tempout)))next
  load(paste("sims/np", scenarios[scen,1], "/lamp", scenarios[scen,2], "/lami", scenarios[scen,3], "/attr", abs(scenarios[scen, 4]), "/rep", i, "/hmmdata.RData", sep=""))
  
  # Ind and group recaptures
  pack_by_trap <- aggregate(apply(data$y.I.obs,c(1,2),sum), by=list(packID=data$packID.obs), sum)
  ...
  if(max(gelmans[i,]>1.1)){next} # maybe add back in later...muy bad
  ...
  true[7]=storeNi[i]=NA # for now #data$Ni
  true[8]=storeSV[i]=NA # for now #sum(data$y.P)
  storeni[i]=dim(data$y.I.obs)[1]
  storenp[i]=dim(pack_by_trap)[1]
...
which(is.na(rowSums(modes))) # 17 convergence or model failures
...
# Convergence/model failure counts
which(is.na(rowSums(modes))) # 15 convergence or model failures
unique(which(gelmans>1.1, arr.ind=T)[,1]) # All convergence failures no model failures
...
# Convergence/model failure counts
which(is.na(rowSums(modes))) # 16 convergence or model failures
unique(which(gelmans>1.1, arr.ind=T)[,1]) # All convergence failures no model failures
...
# Convergence/model failure counts
which(is.na(rowSums(modes))) # 1 convergence or model failures
unique(which(gelmans>1.1, arr.ind=T)[,1]) # All convergence failures no model failures
...
# Convergence/model failure counts
which(is.na(rowSums(modes))) # 7 convergence or model failures
unique(which(gelmans>1.1, arr.ind=T)[,1]) # All convergence failures no model failures
